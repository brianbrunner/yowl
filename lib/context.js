...
/**
 * Retrieve the profile object for the current user's session.
 * Relies on the underlying platform to grab the information.
 * @public
 */

context.profile = function(callback) {
  if (typeof this.platform.profile == "function") {
    this.platform.profile(this.sessionId, callback);
  } else {
    callback(null, {});
  }
}
